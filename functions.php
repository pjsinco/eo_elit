...
/**
 * START -- Elit Comment Modal
 *
 * After a reader posts a comment that hasn't been yet approved,
 * pop open modal that describes our comment moderation policy.
 * 
...
function elit_enqueue_scripts_for_flash_message()
{
  ...
  if ( isset( $_SESSION['flash_message'] ) ) {
    ...
    wp_enqueue_script(
      'vex-js',
      get_template_directory_uri() . '/js/vex.combined.min.js',
      array(),
      false,
      true
    );
    ...
    wp_enqueue_style(
      'vex-style',
      get_template_directory_uri() . '/css/vex.css', 
      array(),
      false
    );
    ...
    wp_enqueue_style(
      'vex-theme-style',
      get_template_directory_uri() . '/css/vex-theme-default.css', 
      array( 'vex-style' ),
      false
    );
  }
...
function elit_flash_moderation_message( $comment_id, $comment_approved )
{
  ...
  $_SESSION['flash_message'] = 'Your comment is awaiting moderation.';
}
...
function elit_check_for_flash_message()
{
  ...
  if ( isset( $_SESSION['flash_message'] ) ) {
    ...
    $output .= "vex.dialog.alert('$message');" . PHP_EOL;
    $output .= '</script>' . PHP_EOL;
